...
import { User, Settings, LogOut, Menu, X, Home, BookOpen, ClipboardList, Target, BookMarked } from 'lucide-react';
    ...
    <div className="min-h-screen bg-[#f8f9fa] overflow-x-hidden">
      <header className="bg-white border-b border-gray-200 fixed top-0 left-0 right-0 z-50">
        <div className="max-w-[1240px] mx-auto pl-2 sm:pl-4 lg:pl-0 pr-4 sm:pr-6 lg:pr-8 h-[56px] md:h-[65px] flex items-center justify-between gap-3">
          <button onClick={() => navigate('/dashboard/candidate')} className="flex items-center hover:opacity-80 transition-opacity flex-shrink-0">
            ...
            <div className="flex items-center gap-1">
              {NAV_LINKS.map(link => (
                <button key={link.path} onClick={() => navigate(link.path)}
                  className={`relative flex items-center px-4 py-2 text-[14px] font-medium rounded-lg transition-colors duration-200 after:absolute after:bottom-0 after:left-4 after:h-[3px] after:rounded-full after:bg-blue-600 after:transition-all after:duration-300 ${isActive(link.path) ? 'text-gray-900 after:w-[calc(100%-2rem)]' : 'text-gray-500 hover:text-gray-900 hover:bg-gray-50 after:w-0 hover:after:w-[calc(100%-2rem)]'}`}>
                  {link.label}
                </button>
              ))}
            </div>
          ...
          <div className="flex items-center gap-1 sm:gap-2 flex-shrink-0">
            <span className="hidden sm:flex items-center gap-1 bg-blue-50 text-blue-700 text-[10px] font-bold px-2.5 py-1 rounded-full border border-blue-100"><Target className="w-3 h-3" /> Candidate</span>
                  ...
                  <div className="p-4 bg-gradient-to-r from-blue-50 to-indigo-50 border-b border-gray-100">
                    <div className="flex items-center gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-sm text-blue-600 font-medium">Independent Candidate</p>
                        <p className="text-xs text-gray-400 truncate">{user?.email}</p>
                      </div>
                    </div>
                  </div>
                  <div className="py-2">
                    {[{icon:User,label:'My Profile',path:'/profile/my-info'},{icon:Settings,label:'Settings',path:'/dashboard/candidate/settings'}].map(item => (
                      <button key={item.label} onClick={() => { setShowUserMenu(false); navigate(item.path); }} className="w-full px-4 py-2.5 text-left flex items-center gap-3 text-gray-700 hover:bg-blue-50 hover:text-blue-700 transition-all">
                        <item.icon className="w-4 h-4 flex-shrink-0" /><span className="text-sm font-medium">{item.label}</span>
                      </button>
                    ))}
                    ...
                    <button onClick={handleLogout} className="w-full px-4 py-2.5 text-left flex items-center gap-3 text-red-600 hover:bg-red-50 transition-all">
                      <LogOut className="w-4 h-4 flex-shrink-0" /><span className="text-sm font-medium">Logout</span>
                    </button>
                  </div>
                </div>
              )}
      ...
      <div className={`fixed top-0 right-0 bottom-0 w-[80vw] max-w-[320px] bg-white z-[70] md:hidden flex flex-col overflow-y-auto transition-transform duration-300 ease-in-out ${showDrawer ? 'translate-x-0' : 'translate-x-full'}`}>
        <div className="p-4 pt-6 pb-4 border-b border-gray-100 relative">
          ...
          <div className="flex items-center gap-3 mb-2">
            <div className="w-14 h-14 rounded-full bg-blue-100 border-2 border-blue-200 flex items-center justify-center flex-shrink-0"><span className="text-xl font-bold text-blue-700">{getInitials()}</span></div>
            <div className="min-w-0">
              <p className="font-bold text-gray-900 text-[15px] truncate">{getName()}</p>
              <p className="text-[12px] text-blue-600 font-medium">Independent Candidate</p>
        ...
        <nav className="flex-1 px-2 py-3 space-y-0.5">
          {DRAWER_LINKS.map(link => { const active = isActive(link.path); return (
            <button key={link.label} onClick={() => navigate(link.path)} className={`w-full flex items-center gap-3 px-3 py-3 rounded-xl text-left transition-colors ${active ? 'bg-blue-50 text-blue-700' : 'text-gray-800 hover:bg-gray-50'}`}>
              <link.icon className={`w-5 h-5 ${active ? 'text-blue-600' : 'text-gray-500'}`} /><span className="text-[14px] font-medium">{link.label}</span>
            </button>
          );})}
        </nav>
        <div className="border-t border-gray-100 p-3">
          <button onClick={handleLogout} className="w-full flex items-center gap-3 px-3 py-2.5 text-red-600 hover:bg-red-50 rounded-xl transition-colors"><LogOut className="w-4 h-4" /><span className="text-[14px] font-medium">Logout</span></button>
        </div>
      </div>
            ...
            <div className="max-w-[1240px] mx-auto px-4 sm:px-6 lg:px-8 py-8">
              <div className="flex flex-col md:flex-row items-center justify-between gap-4">
                <div className="flex items-center gap-2">
                  <div className="w-8 h-8 bg-gradient-to-br from-blue-500 to-cyan-400 rounded-lg flex items-center justify-center"><span className="text-white font-black text-sm">I</span></div>
                  <span className="font-black text-white text-[15px]">ICL</span>
                  <span className="text-slate-400 text-[12px]">Innovation &amp; Career Launch</span>
                </div>
                <p className="text-[12px] text-slate-500">© {new Date().getFullYear()} ICL Academy. All rights reserved.</p>
              </div>
            </div>
          </footer>
        </div>
