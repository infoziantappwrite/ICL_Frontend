// pages/Candidate/Assessments/TakeAssessment.jsx
...
const QuestionPill = ({ number, status, onClick, isCurrent }) => {
  const base = 'w-8 h-8 rounded-full flex items-center justify-center text-3 font-bold cursor-pointer transition-all duration-150 select-none';
  const styles = {
...
const TagChip = ({ label }) => {
  ...
  return (
    <span className={`text-2.5 font-bold uppercase tracking-wider px-2 py-0.5 rounded border ${cls}`}>
      {label}
    </span>
  );
};
  ...
  <div className="rounded-lg border border-gray-200 overflow-hidden bg-white shadow-sm">
    ...
    <div className="grid grid-cols-2 divide-x divide-gray-200">
      <div className="p-3">
        ...
        <pre className="text-3 font-mono text-gray-800 whitespace-pre-wrap leading-relaxed">{input}</pre>
      </div>
      <div className="p-3">
        <p className="text-[9px] font-bold text-gray-400 uppercase tracking-widest mb-1.5">Output</p>
        <pre className="text-3 font-mono text-emerald-600 whitespace-pre-wrap leading-relaxed">{output}</pre>
      </div>
    </div>
    {explanation && (
      <div className="px-3 py-2 border-t border-gray-100 bg-gray-50/50">
        <p className="text-2.5 font-bold text-gray-400 uppercase tracking-widest mb-1">Explanation</p>
        <p className="text-3 text-gray-600 italic leading-relaxed">{explanation}</p>
      </div>
    )}
  </div>
);
...
const SqlSampleCase = ({ index, setupSql, output, explanation }) => (
  <div className="rounded-lg border border-amber-200 overflow-hidden bg-white shadow-sm">
    <div className="bg-amber-50 px-3 py-1.5 border-b border-amber-200 flex items-center justify-between">
      ...
      <span className="text-2.5 text-amber-500 font-medium">Schema is pre-loaded — write only the query</span>
    </div>
    {setupSql && (
      <div className="p-3 border-b border-amber-100 bg-slate-950">
        ...
        <pre className="text-3 font-mono text-emerald-300 whitespace-pre-wrap leading-relaxed">{setupSql}</pre>
      </div>
    )}
    <div className="p-3">
...
const FullLoader = ({ title, sub, pct }) => (
  <div className="h-screen w-screen bg-white flex items-center justify-center">
    <div className="flex flex-col items-center gap-4 text-center">
      ...
      <h2 className="text-4 font-bold text-gray-900">{title}</h2>
  ...
  return (
    <div className={`fixed top-4 left-1/2 -translate-x-1/2 z-[9999] flex items-start gap-3 px-5 py-4 rounded-2xl shadow-2xl border max-w-md w-full mx-4 animate-in slide-in-from-top-4 duration-300
      ...
      <div className="flex-1 min-w-0">
        <p className="font-bold text-3.5 leading-tight">
          {isCritical ? '⚠ Proctoring Violation Detected' : '⚠ Warning'}
        </p>
        <p className="text-3 opacity-90 mt-0.5">{violation.label}</p>
  ...
  >
    <div className="w-full max-w-sm mx-4">
      ...
      <div className="bg-amber-500 rounded-t-2xl px-5 py-4 flex items-center gap-3">
        ...
        <div>
          <p className="text-white font-black text-3.5">You Exited Fullscreen</p>
      ...
      <div className="bg-white rounded-b-2xl shadow-2xl px-6 py-6 text-center">
        ...
        <p className="text-3 text-gray-400 mb-6">
        ...
        <button
          ...
          className="w-full py-3.5 bg-blue-600 hover:bg-blue-700 text-white rounded-xl font-black text-3.5 shadow-md transition-all active:scale-95 flex items-center justify-center gap-2 mb-3"
        >
        ...
        <p className="text-2.5 text-gray-400 mt-4">
...
const FlaggedNoticeBanner = ({ count }) => (
  <div className="fixed top-3 left-1/2 -translate-x-1/2 z-[9990] max-w-sm w-full px-4">
    <div className="bg-amber-50 border border-amber-300 rounded-xl px-4 py-3 shadow-lg flex items-start gap-3">
      ...
      <div>
        <p className="text-3 font-bold text-amber-800">Proctoring Violation Noted</p>
...
const CameraViolationModal = ({ violationCount, onDismiss }) => (
  <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-[9997] flex items-center justify-center p-4">
    <div className="bg-white rounded-2xl shadow-2xl max-w-sm w-full p-8 text-center">
      ...
      <h3 className="font-bold text-4.5 text-gray-900 mb-2">Camera Violation Detected</h3>
      ...
      <div className="bg-amber-50 border border-amber-200 rounded-xl px-4 py-3 mb-5">
        <p className="text-3 font-bold text-amber-700">
      ...
      <button
        onClick={onDismiss}
        className="w-full py-3.5 bg-blue-600 text-white rounded-xl font-bold text-3.5 hover:bg-blue-700 shadow-sm transition-colors"
      >
...
const ProctoringBadge = ({ warningCount, camViolationCount }) => {
  ...
  return (
    <div className={`hidden md:flex items-center gap-1.5 px-2.5 py-1.5 rounded-full text-2.5 font-bold border
        ...
        {scanning && (
          <div className="absolute inset-0 flex items-center justify-center bg-black/30">
            <div className="flex flex-col items-center gap-2">
              ...
              <p className="text-white text-2.5 font-bold">Loading AI…</p>
            </div>
          </div>
        )}
        {/* Status badge */}
        <div className={`absolute bottom-2 left-1/2 -translate-x-1/2 px-2.5 py-1 rounded-full text-2.5 font-bold whitespace-nowrap ${badgeBg} text-white`}>
          {badgeText}
        </div>
      </div>

      {/* Guidance text */}
      <p className={`text-[11px] font-medium text-center max-w-65 ${guideColor}`}>
        {guideText}
      </p>
    </div>
  );
...
const TakeAssessment = () => {
    ...
    assessmentAttemptAPI.getMyAttempts(assessmentId)
      .then(res => {
        if (res.success && res.attempts?.some(a => a.status === 'submitted'))
          navigate('/dashboard/candidate/assessments/history', { replace: true });
      })
      .catch(() => {});
    ...
    return (
      <div className="fixed inset-0 bg-[#0f172a] z-[9999] flex flex-col items-center justify-center p-8 text-center overflow-hidden">
        ...
        <div className="relative z-10 flex flex-col items-center">
          <div className="w-24 h-24 bg-linear-to-tr from-blue-600/20 to-cyan-500/20 rounded-[2rem] flex items-center justify-center mb-8 border border-white/10 backdrop-blur-sm shadow-2xl">
          ...
          <div className="w-16 h-1 bg-linear-to-r from-blue-600 to-cyan-500 rounded-full mb-8" />
      ...
      return (
        <div className="h-screen w-screen bg-white flex items-center justify-center">
          <div className="text-center max-w-sm">
            ...
            <h2 className="text-4 font-bold text-gray-900 mb-2">Error</h2>
      ...
      <div className="min-h-screen bg-[#F8F9fa] font-sans text-gray-800" style={{ fontFamily: "'Inter', 'IBM Plex Sans', sans-serif" }}>
        ...
        <header className="bg-white border-b border-gray-200 px-6 py-4 flex items-center justify-between shrink-0 sticky top-0 z-10 shadow-sm">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-4 font-bold text-gray-900 truncate max-w-75">{skill}</h1>
          ...
          <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-green-50 border border-green-200 shrink-0">
            ...
            <span className="text-2.5 font-bold text-green-700 uppercase tracking-wider">Secured</span>
          </div>
        </header>
        ...
        <main className="w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 lg:py-10">
          <div className="flex flex-col lg:flex-row gap-6 lg:gap-8 items-start">
            ...
            <div className="flex-1 w-full bg-white p-6 lg:p-8 rounded-2xl border border-gray-200 shadow-sm">
              ...
              <div className="space-y-8">
                ...
                {CAMERA_PROCTORING_ENABLED && (
                  <section>
                    <div className={`rounded-xl p-4 border transition-all ${
                      ...
                      {cameraPermStatus === 'granted' ? (
                        <div>
                          <p className="text-3 text-green-700 font-medium mb-3">✓ Camera permission granted. Check your position in the preview below.</p>
                          {/* ── Live camera preview ── */}
                          <BriefingCameraPreview camera={camera} />
                        </div>
                      ) : cameraPermStatus === 'denied' ? (
                        <div>
                          <p className="text-3 text-red-700 font-medium mb-1">Camera access was denied. Allow camera access in your browser settings and reload this page.</p>
                          <p className="text-[11px] text-red-600">Click the camera icon in your address bar → Allow → Reload.</p>
                        </div>
                      ) : cameraPermStatus === 'error' ? (
                        <div>
                          <p className="text-3 text-red-700 font-medium mb-1">Camera error. Another app (e.g. Teams) may be blocking it.</p>
                          <button
                            onClick={async () => {
                            ...
                            className="mt-2 px-3 py-1.5 bg-red-600 text-white text-3 font-bold rounded-lg hover:bg-red-700 transition-colors"
                          >Retry Camera Access</button>
                        </div>
                      ) : (
                        <div>
                          <p className="text-3 text-amber-700 font-medium mb-3">
                  ...
                  <section>
                    <div className="bg-blue-50 border border-blue-200 rounded-xl p-4 flex items-start gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-3 text-blue-700">
                ...
                {!PROCTORING_ENABLED && (
                  <section>
                    <div className="bg-yellow-50 border border-yellow-300 rounded-xl p-4">
                      ...
                      <p className="text-3 text-yellow-700 font-medium">
                ...
                <section>
                  ...
                  <ul className="text-3.5 text-gray-600 space-y-3">
                    {[
                ...
                {briefingInfo?.instructions && (
                  <section>
                    ...
                    <div className="text-3.5 text-gray-700 bg-gray-50/80 p-5 rounded-xl border border-gray-100 whitespace-pre-wrap leading-relaxed">
                      {briefingInfo.instructions}
                    </div>
                  </section>
                )}
              ...
              <div className="bg-blue-50/50 border border-blue-100 rounded-xl p-5 mt-8 flex gap-4">
                ...
                <div>
                  <h4 className="font-bold text-blue-900 text-3.5 mb-1">Important Note</h4>
            ...
            <div className="w-full lg:w-90 shrink-0 sticky top-24">
              <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
                ...
                <div className="bg-linear-to-r from-blue-600 to-cyan-600 p-6">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <h3 className="font-bold text-white text-4 leading-tight">{skill}</h3>
                      <span className="text-3 text-blue-100">Comprehensive Assessment</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="p-6">
                  ...
                  <div className="space-y-0 divide-y divide-gray-100 mb-6">
                    {[
                      ...
                      <div key={label} className="flex items-center justify-between py-3.5">
                        ...
                        <span className="font-bold text-gray-900 text-3.5">{value}</span>
                      </div>
                    ))}
                    <div className="flex items-center justify-between py-3.5">
                      ...
                      <span className="flex items-center gap-1 font-bold text-green-700 bg-green-50 px-2.5 py-0.5 rounded border border-green-200 text-2.5 uppercase tracking-wider">
                  ...
                  <div className="space-y-3">
                    <button
                      ...
                      className="w-full flex items-center justify-center gap-2 py-3.5 bg-linear-to-r from-blue-600 to-cyan-600 text-white font-bold rounded-xl hover:shadow-lg hover:shadow-blue-200 transition-all text-3.5 disabled:opacity-50 disabled:cursor-not-allowed disabled:shadow-none"
                    >
        ...
        <div className="text-center max-w-md w-full">
          <Card className="p-0 overflow-hidden">
            <div className="py-12 px-8 bg-green-50 border-b border-green-100">
              ...
              <h2 className="text-5 font-bold text-gray-900 mb-2">Assessment Submitted!</h2>
              <p className="text-3.5 text-gray-500">
            ...
            <div className="p-8 flex gap-3">
              <button onClick={() => navigate('/dashboard/candidate/assessments/history')}
                className="flex-1 py-3 rounded-xl border border-gray-200 text-gray-600 font-bold text-3.5 hover:bg-gray-50 transition-colors">
                View History
              </button>
              <button onClick={() => navigate('/dashboard/candidate/assessments')}
                className="flex-1 py-3 rounded-xl bg-blue-600 text-white font-bold text-3.5 hover:bg-blue-700 transition-colors shadow-sm">
                Back to Assessments
              </button>
            </div>
          </Card>
  ...
  return (
    <div className="flex flex-col h-screen w-screen overflow-hidden bg-white" style={{ fontFamily: "'Inter', 'IBM Plex Sans', sans-serif", userSelect: 'none' }}>
        ...
        <div className="flex items-center justify-between px-5 gap-4" style={{ height: '52px' }}>
          ...
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="min-w-0">
              <span className="text-3.5 font-bold text-gray-900 truncate max-w-60 block leading-tight">
          ...
          <div className="flex items-center gap-2 shrink-0">
            ...
            <button
              ...
              className="flex items-center gap-1.5 px-4 py-2 rounded-lg bg-linear-to-r from-blue-600 to-cyan-600 hover:shadow-md text-white text-3 font-bold transition-shadow"
            >
      ...
      {/* Error bar */}
      {error && (
        <div className="px-5 py-2 bg-red-50 border-b border-red-100 flex items-center gap-2 text-3 font-medium text-red-700">
      ...
      <div className="flex flex-1 min-h-0 overflow-hidden">
        ...
        <main ref={questionScrollRef} className="flex-1 overflow-y-auto min-h-0 bg-gray-50/40">
          <div className="max-w-180 mx-auto px-8 py-6">
              ...
              <div className={`mb-5 rounded-2xl overflow-hidden shadow-sm border
                ...
                <div className={`px-5 py-3 flex items-center justify-between
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-white font-bold text-3.5 leading-tight">
                  ...
                  <span className={`text-2.5 font-black px-2.5 py-1 rounded-full uppercase tracking-wider
                    ${currentSection.section_type === 'coding'
            ...
            <div className="mb-6">
              <div className="flex items-start justify-between gap-3 mb-3">
                <div className="flex-1 min-w-0">
                  ...
                  <h1 className="text-4.5 font-bold text-gray-900 leading-snug">{questionText}</h1>
                </div>
                <button
            ...
            {isCodingQ && (
              <>
                    ...
                    {q.input_format && (
                      <div className="rounded-xl border border-gray-200 overflow-hidden bg-white shadow-sm">
                        <div className="bg-gray-50 px-3 py-2 border-b border-gray-200">
                          <span className="text-2.5 font-bold text-gray-500 uppercase tracking-wider">Input Format</span>
                        </div>
                        <div className="p-3">
                          <p className="text-3 text-gray-600 leading-relaxed font-mono whitespace-pre-wrap">{q.input_format}</p>
                        </div>
                      </div>
                    )}
                    {q.output_format && (
                      <div className="rounded-xl border border-gray-200 overflow-hidden bg-white shadow-sm">
                        <div className="bg-gray-50 px-3 py-2 border-b border-gray-200">
                          <span className="text-2.5 font-bold text-gray-500 uppercase tracking-wider">Output Format</span>
                        </div>
                        <div className="p-3">
                          <p className="text-3 text-gray-600 leading-relaxed font-mono whitespace-pre-wrap">{q.output_format}</p>
                        </div>
                      </div>
                    )}
                  </section>
                  ...
                  <section className="mb-5">
                    <div className="rounded-xl border border-gray-200 overflow-hidden bg-white shadow-sm">
                      <div className="bg-gray-50 px-4 py-2.5 border-b border-gray-200 flex items-center justify-between">
                        <span className="text-2.5 font-bold text-gray-500 uppercase tracking-wider">Constraints</span>
                      ...
                      <div className="p-4">
                        <pre className="text-3 font-mono text-gray-600 whitespace-pre-wrap">{q.constraints}</pre>
                      </div>
                    </div>
                  </section>
                )}
                ...
                {q.test_cases?.filter(tc => !tc.is_hidden).length > 0 && (
                  <section className="mb-5">
                    <div className="flex items-center gap-2 mb-3">
                      <span className="text-3 font-bold text-gray-900">Sample Test Cases</span>
                      <span className="text-2.5 font-bold bg-blue-50 text-blue-600 border border-blue-200 px-2 py-0.5 rounded-full">
                ...
                <div className="text-3 text-gray-400 font-medium italic flex items-center gap-2 mb-4">
              ...
              <div className="bg-white rounded-2xl border border-gray-100 shadow-sm px-6 py-5 mb-4">
                ...
                {(q.type === 'single_answer' || q.type === 'multiple_answer') && opts.length > 0 && (
                  <div className="space-y-2.5 mb-0">
                    ...
                    {q.type === 'multiple_answer' && (
                      <p className="text-3 font-medium text-amber-700 bg-amber-50 border border-amber-200 px-3 py-2 rounded-lg flex items-center gap-2 mt-3">
            ...
            <div className="mt-4">
              ...
              <button onClick={() => setCurrentIdx(i => Math.max(0, i - 1))} disabled={currentIdx === 0}
                className="flex items-center gap-1.5 px-4 py-2 rounded-xl border border-gray-200 text-3 font-bold text-gray-500 hover:border-gray-400 hover:text-gray-700 disabled:opacity-30 disabled:cursor-not-allowed transition-colors bg-white shadow-sm">
                <ChevronLeft className="w-3.5 h-3.5" /> Previous
              </button>
              <div className="flex items-center gap-2">
                <button onClick={() => handleSkip(qid)}
                  className="flex items-center gap-1 px-3 py-2 rounded-xl border border-gray-200 text-3 font-bold text-gray-500 hover:border-gray-400 transition-colors bg-white shadow-sm">
                  Skip <SkipForward className="w-3 h-3" />
                </button>
                {currentIdx < total - 1 ? (
                  <button onClick={() => setCurrentIdx(i => Math.min(i + 1, total - 1))}
                    className="flex items-center gap-1.5 px-5 py-2 rounded-xl bg-blue-600 text-white text-3 font-bold hover:bg-blue-700 transition-colors shadow-sm shadow-blue-200">
                    Next <ChevronRight className="w-3.5 h-3.5" />
                  </button>
                ) : (
                  <button onClick={() => setSubmitConfirm(true)}
                    className="flex items-center gap-1.5 px-5 py-2 rounded-xl bg-green-600 text-white text-3 font-bold hover:bg-green-700 transition-colors shadow-sm shadow-green-200">
        ...
        {isCodingQ ? (
          ...
          <aside className="hidden lg:flex flex-col bg-white border-l border-gray-100 shadow-[inset_1px_0_0_rgba(0,0,0,0.04)] shrink-0 overflow-hidden" style={{ width: '280px' }}>
            <div className="w-70 flex flex-col h-full overflow-hidden">
              {/* Palette header */}
              <div className="px-4 py-3.5 border-b border-gray-100 bg-linear-to-b from-white to-gray-50/50">
              ...
              <div className="px-3 py-2.5 grid grid-cols-2 gap-1.5 border-b border-gray-100 bg-gray-50/50">
                {[
                  ...
                  <div key={l.label} className="flex items-center gap-1.5 bg-white rounded-lg px-2 py-1.5 border border-gray-100">
                    ...
                    <span className="text-2.5 font-semibold text-gray-500">{l.label}</span>
                  </div>
                ))}
              </div>
                      ...
                      <div key={group.id} className={`rounded-xl overflow-hidden border transition-all ${
                        ...
                        {group.section && (
                          <button
                            ...
                            <span className={`text-2.5 font-bold truncate flex-1 ${isCoding ? 'text-violet-700' : 'text-blue-700'}`}>
                              {group.section.section_title}
                            </span>
                            <span className={`text-2 font-black uppercase px-1.5 py-0.5 rounded-full ${isCoding ? 'bg-violet-100 text-violet-600' : 'bg-blue-100 text-blue-600'}`}>
                              {isCoding ? 'Code' : 'Quiz'}
                            </span>
                          </button>
                        )}
              ...
              <div className="p-3 border-t border-gray-100 bg-white">
                <button
                  onClick={() => setSubmitConfirm(true)}
                  className="w-full py-3 bg-linear-to-r from-blue-600 to-cyan-600 text-white rounded-xl text-[13px] font-bold hover:shadow-md hover:shadow-blue-200 transition-all flex items-center justify-center gap-2">
      ...
      {submitConfirm && (
        <div className="fixed inset-0 bg-black/30 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <Card className="w-full max-w-sm p-8 text-center animate-in fade-in zoom-in duration-200">
            ...
            <h3 className="font-bold text-4.5 text-gray-900 mb-2">Submit Assessment?</h3>
            <p className="text-3.5 text-gray-500 mb-3">
            ...
            {total - answeredCount > 0 && (
              <div className="bg-red-50 text-red-600 font-medium text-3 py-2 px-3 rounded-lg mb-3 border border-red-100">
                {total - answeredCount} unanswered question{total - answeredCount !== 1 ? 's' : ''} will be marked incorrect.
              </div>
            )}
            {guard.warningCount > 0 && (
              <div className="bg-amber-50 text-amber-700 font-medium text-3 py-2 px-3 rounded-lg mb-2 border border-amber-200">
                🖥 {guard.warningCount} browser violation{guard.warningCount !== 1 ? 's' : ''} recorded — noted for review.
              </div>
            )}
            {camViolationCount > 0 && (
              <div className="bg-amber-50 text-amber-700 font-medium text-3 py-2 px-3 rounded-lg mb-3 border border-amber-200">
