...
import {
  ...
  Tag, BarChart3, Zap, FileText, Lock,
  RefreshCw, MessageSquare, ThumbsUp, Home,
} from 'lucide-react';
...
const CandidateCourseDetail = () => {
  ...
  const learnCount   = parseInt(course.enrollmentCount || 0).toLocaleString();

  // ── CTA button — reused on desktop card + mobile bar ──────────────────
  const CTAButton = ({ compact = false }) =>
    !isEnrolled ? (
      <button
        onClick={handleEnroll}
        disabled={enrolling || course.status !== 'Active'}
        className={`w-full bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white font-bold rounded-xl flex items-center justify-center gap-2 transition-all disabled:opacity-60 disabled:cursor-not-allowed shadow-md ${
      ...
      <button
        onClick={() => navigate(`/dashboard/candidate/courses/${courseId}/learn`)}
        className={`w-full bg-gray-900 hover:bg-gray-800 text-white font-bold rounded-xl flex items-center justify-center gap-2 transition-all shadow-md ${
          compact ? 'py-2.5 text-[13px]' : 'py-3.5 text-[15px]'
        }`}
      >
  ...
  return (
    <CandidateLayout title={course.title}>
      ...
      {/* ── Toast notification ── */}
      {toast && (
        <div className={`fixed top-4 right-4 z-50 flex items-center gap-2.5 px-4 py-3 rounded-xl shadow-xl text-[13px] font-bold ${
      ...
      <div className="bg-white border-b border-gray-100">
        <div className="max-w-310 mx-auto px-3 sm:px-4 md:px-6 lg:px-8 py-2">
      ...
      {/* ── Mobile banner image ── */}
      {hasImage && (
        <div className="lg:hidden w-full h-44 sm:h-56 overflow-hidden relative">
          <img
        ...
        <div className="max-w-310 mx-auto w-full px-3 sm:px-4 md:px-6 lg:px-8 py-5 md:py-12 flex flex-col lg:flex-row items-start gap-6 lg:gap-12">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex flex-wrap gap-1.5 mb-3">
              <span className="bg-white border border-blue-200 text-blue-700 text-[11px] font-bold uppercase tracking-wider px-2.5 py-1 rounded-full shadow-sm">
                {course.category || 'Course'}
              </span>
              <span className="bg-indigo-50 border border-indigo-200 text-indigo-700 text-[11px] font-bold uppercase tracking-wider px-2.5 py-1 rounded-full">
                {course.level || 'All Levels'}
              </span>
              <span className="bg-emerald-50 border border-emerald-200 text-emerald-700 text-[11px] font-bold uppercase tracking-wider px-2.5 py-1 rounded-full">
                Free
              </span>
              {isEnrolled && (
                <span className="bg-gray-900 text-white text-[11px] font-bold uppercase tracking-wider px-2.5 py-1 rounded-full">
            ...
            {/* Title */}
            <h1 className="text-[20px] sm:text-[24px] md:text-[30px] font-bold text-gray-900 leading-tight mb-2">
              {course.title}
            </h1>
            ...
            <p className="text-sm text-gray-500 leading-relaxed mb-4">
              {course.shortDescription ||
                ...
                'Master this topic with hands-on practice and expert guidance.'}
            </p>
            ...
            <div className="flex flex-wrap gap-2 mt-5 pt-4 border-t border-gray-200/60">
              {[
                ...
                <div
                  key={label}
                  className="flex items-center gap-1.5 bg-white border border-gray-200 px-3 py-1.5 rounded-lg shadow-sm"
                >
            ...
            <div className="flex items-center gap-2 mt-4">
              {partnerLogo ? (
                <img
                  src={partnerLogo}
                  alt={partnerName}
                  className="w-7 h-7 rounded-lg object-contain border border-gray-200 shrink-0"
                />
              ) : (
                <div className="w-7 h-7 rounded-lg bg-blue-100 border border-blue-200 text-blue-700 font-bold text-[11px] flex items-center justify-center shrink-0">
                  {partnerName.charAt(0)}
                </div>
              )}
              <span className="text-[11px] text-gray-500">
                Taught by <span className="font-bold text-gray-800">{partnerName}</span>
              </span>
            </div>
          </div>
          ...
          <div className="hidden lg:block w-full lg:w-90 shrink-0">
            <div className="bg-white border border-gray-200 rounded-2xl shadow-[0_8px_30px_rgba(0,0,0,0.08)] overflow-hidden">
              ...
              <div className="p-5">
                <h3 className="text-[17px] font-bold text-gray-900 mb-1">Start Learning</h3>
                <p className="text-[13px] text-gray-500 mb-4">
                  Join <strong className="text-blue-700">{learnCount}</strong> learners already enrolled.
                </p>
        ...
        <div className="max-w-310 mx-auto px-3 md:px-6 lg:px-8">
          ...
          <div className="flex gap-3 overflow-x-auto hide-scrollbar py-2.5 lg:hidden">
            {[
              ...
              <div
                key={value}
                className="flex items-center gap-1.5 bg-gray-50 border border-gray-200 rounded-full px-3 py-1.5 shrink-0"
              >
          ...
          <div className="hidden lg:grid grid-cols-4 divide-x divide-gray-100 py-4">
            {[
              ...
              { label: `${course.level || 'All'} level`,                               sub: 'Recommended experience'                   },
          ...
          <nav className="flex gap-1 overflow-x-auto hide-scrollbar">
            {TABS.map(tab => (
              <button
                ...
                className={`whitespace-nowrap py-3 px-3 md:px-4 border-b-2 text-[12px] md:text-[13px] font-bold shrink-0 transition-colors ${
                  activeTab === tab
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            {(activeTab === 'About' || activeTab === 'Outcomes') && (
              <div className="space-y-6 md:space-y-10">
                ...
                <section>
                  <h2 className="text-[17px] md:text-[22px] font-bold text-gray-900 mb-3">About this course</h2>
                ...
                <section>
                  <h2 className="text-[15px] md:text-[17px] font-bold text-gray-900 mb-3">Skills you'll gain</h2>
                  <div className="flex flex-wrap gap-2">
                    {(course.learningOutcomes?.length > 0
                      ? course.learningOutcomes
                      : ['Android Development', 'React.js', 'Cross Platform', 'UI Design']
                    ).map((skill, i) => (
                      <span
                        key={i}
                        className="bg-gray-100 text-gray-800 text-[12px] md:text-[13px] font-medium px-3 py-1.5 rounded-full border border-gray-200"
                      >
                        {skill}
                      </span>
                    ))}
                ...
                {course.tags?.length > 0 && (
                  <section>
                    <h2 className="text-[15px] md:text-[17px] font-bold text-gray-900 mb-3">Tools you'll use</h2>
                    <div className="flex flex-wrap gap-2">
                      {course.tags.map(tag => (
                        <span
                          key={tag}
                          className="bg-blue-50 text-blue-700 text-[12px] md:text-[13px] font-medium px-3 py-1.5 rounded-full border border-blue-100"
                        >
                          {tag}
                        </span>
                      ))}
            ...
            {activeTab === 'Modules' && (
              <section>
                <h2 className="text-[17px] md:text-[22px] font-bold text-gray-900 mb-4">Course Modules</h2>
                      ...
                      >
                        ...
                        >
                          <div className="flex items-center gap-3 min-w-0">
                            <div className={`w-7 h-7 md:w-8 md:h-8 rounded-lg flex items-center justify-center text-[11px] md:text-[12px] font-bold shrink-0 ${
                              expandedModule === i ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-600'
                            }`}>
                              {i + 1}
                            </div>
                            <div className="min-w-0">
                              <p className="text-[13px] md:text-[15px] font-bold text-gray-900 truncate">
                                {mod.module}
                              </p>
                          ...
                          <ChevronRight className={`w-4 h-4 text-gray-400 shrink-0 transition-transform ${
                            expandedModule === i ? 'rotate-90 text-blue-600' : ''
                          }`} />
                        </button>
                            ...
                            {mod.topics?.length > 0 ? (
                              <ul className="mt-2 space-y-1.5">
                                {mod.topics.map((topic, j) => (
                                  <li
                                    key={j}
                                    className="flex items-start gap-2 text-[12px] md:text-[13px] text-gray-700 py-1.5 border-b border-gray-100 last:border-0"
                                  >
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-5 shadow-sm">
              ...
              <div className="flex items-center gap-3">
                {partnerLogo ? (
                  ...
                  <div className="w-10 h-10 rounded-full bg-blue-100 border border-blue-200 flex items-center justify-center text-blue-700 font-bold text-[15px] shrink-0">
                    {partnerName.charAt(0)}
                  </div>
                )}
                <div>
                  <p className="text-[13px] font-bold text-blue-700">
                    {course.instructor?.name || partnerName}
                  </p>
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-5 shadow-sm">
              ...
              <div className="space-y-3">
                {[
                  ...
                  ...(course.certificateProvided
                    ? [{ icon: Award, title: 'Certificate included', sub: 'Share on LinkedIn' }]
                    : []),
                  ...(course.language
                    ? [{ icon: FileText, title: course.language, sub: 'Course language' }]
                    : []),
            ...
            {course.assessmentId && (
              <div className="bg-blue-50 border border-blue-200 rounded-xl p-4">
                ...
                <p className="text-[12px] text-blue-700 leading-relaxed">
                  Validates your skills on completion!
                </p>
              </div>
            )}
          </div>
