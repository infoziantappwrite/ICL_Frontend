// src/pages/Candidate/CandidateSettings.jsx
...
const Toggle = ({ checked, onChange }) => (
  <button
    ...
    className={"relative w-11 h-6 rounded-full transition-all duration-200 shrink-0 " + (checked ? 'bg-linear-to-r from-blue-600 to-cyan-500' : 'bg-gray-200')}
  >
...
const SettingRow = ({ icon: Icon, title, description, children, danger }) => (
  <div className={"flex items-center justify-between p-3.5 rounded-xl transition-all " + (danger ? 'hover:bg-red-50' : 'hover:bg-gray-50')}>
    <div className="flex items-center gap-3 flex-1 min-w-0">
      <div className={"w-8 h-8 rounded-lg flex items-center justify-center shrink-0 " + (danger ? 'bg-red-100' : 'bg-blue-50')}>
...
const SectionCard = ({ title, icon: Icon, children }) => (
  <div className="bg-white/80 backdrop-blur-xl rounded-2xl shadow-md border border-white/60 overflow-hidden">
    <div className="flex items-center gap-3 px-5 py-4 border-b border-gray-100">
      <div className="w-8 h-8 bg-linear-to-br from-blue-600 to-cyan-500 rounded-xl flex items-center justify-center shadow-sm">
...
const CandidateSettings = () => {
  ...
  return (
    <CandidateLayout title="Settings">
      <div className="max-w-2xl mx-auto px-4 pb-8 space-y-6 pt-6">

        {/* Header */}
        <div className="flex items-center gap-3 mb-2">
          <div className="w-11 h-11 bg-linear-to-br from-blue-600 to-cyan-500 rounded-2xl flex items-center justify-center shadow-lg">
            <Settings className="w-5 h-5 text-white" />
          </div>
          <div>
            <h1 className="text-xl font-bold text-gray-900">Settings</h1>
            <p className="text-xs text-gray-500">Manage your candidate account preferences</p>
          </div>
        </div>

        {/* Account Info Banner */}
        <div className="bg-linear-to-r from-blue-600 via-blue-500 to-cyan-500 rounded-2xl p-5 text-white shadow-xl shadow-blue-500/20 relative overflow-hidden">
          ...
          <div className="relative z-10 flex items-center justify-between">
            <div className="flex items-center gap-3">
              <div className="w-12 h-12 bg-white/20 rounded-xl flex items-center justify-center font-bold text-lg">
                {user?.fullName ? user.fullName.substring(0, 2).toUpperCase() : 'CA'}
              </div>
              <div>
                <p className="font-bold">{user?.fullName || user?.name || 'Candidate'}</p>
                <p className="text-blue-100 text-sm">{user?.email}</p>
                <span className="mt-1 inline-block text-xs bg-white/20 px-2 py-0.5 rounded-full">Candidate</span>
              </div>
            </div>
            <button
              onClick={() => navigate('/dashboard/candidate/profile/edit')}
              ...
              {[
                ...
                <button
                  ...
                  className={"flex flex-col items-center gap-2 p-3 rounded-xl border transition-all " + (
                    theme === key
                      ? 'border-blue-400 bg-linear-to-br from-blue-50 to-cyan-50 text-blue-700'
