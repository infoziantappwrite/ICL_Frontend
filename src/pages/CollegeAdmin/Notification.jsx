              ...
              <div className="flex flex-col md:flex-row gap-3">
                <div className="flex-1 relative">
                  ...
                  <input type="text" placeholder="Search notifications…" value={searchTerm}
                    ...
                    className="w-full pl-10 pr-4 py-2.5 text-[13px] border border-gray-200 rounded-lg focus:outline-none focus:ring-1 focus:ring-blue-500 transition-shadow bg-white" />
                  {searchTerm && (
                    <button onClick={() => setSearchTerm('')} className="absolute right-3 top-1/2 -translate-y-1/2 bg-gray-100 p-1.5 rounded-full text-gray-400 hover:text-gray-600">
                      <X className="w-3 h-3" />
                    </button>
                  )}
                </div>
                <div className="flex items-center gap-2">
                  <select value={filterType} onChange={e => setFilterType(e.target.value)}
                    className="px-4 py-2.5 text-[13px] font-bold text-gray-700 border border-gray-200 rounded-lg focus:outline-none focus:ring-1 focus:ring-blue-500 shadow-sm cursor-pointer bg-white w-full md:w-auto">
              ...
              {notifications.length > 0 && (
                <div className="flex justify-between items-center mt-3 pt-3 border-t border-gray-100">
                  <button onClick={markAllRead}
                    className="inline-flex items-center gap-1.5 px-3 py-1.5 text-[12px] font-bold text-blue-600 hover:text-blue-700 hover:underline transition-colors">
              ...
              {filtered.length > 0 ? (
                <div className="divide-y divide-gray-50">
                  {filtered.map(n => {
                    ...
                    return (
                      <div key={n.id}
                        className={`p-4 hover:bg-gray-50/70 transition-colors cursor-pointer group ${!n.read ? 'bg-blue-50/10' : 'bg-white'}`}
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-1 sm:gap-2 mb-1.5">
                              ...
                              <div className="flex items-center gap-2 flex-shrink-0 self-start sm:self-auto">
                                <span className="text-[11px] text-gray-400 font-medium">{timeAgo(n.timestamp)}</span>
                            ...
                            <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                              <button onClick={e => { e.stopPropagation(); toggleStar(n.id); }}
                                className={`p-1.5 rounded transition-colors ${n.starred ? 'text-amber-500 bg-amber-50' : 'text-gray-400 hover:text-amber-500 hover:bg-gray-100'}`} title="Star Notification">
                                {n.starred ? <Star className="w-4 h-4" fill="currentColor" /> : <Star className="w-4 h-4" />}
                              </button>
                              {!n.read
                                ? <button onClick={e => { e.stopPropagation(); markRead(n.id); }} className="p-1.5 text-gray-400 hover:text-blue-600 hover:bg-blue-50 rounded transition-colors" title="Mark as Read"><Check className="w-4 h-4" /></button>
                                : <button onClick={e => { e.stopPropagation(); markUnread(n.id); }} className="p-1.5 text-gray-400 hover:text-gray-600 hover:bg-gray-100 rounded transition-colors" title="Mark as Unread"><BellOff className="w-4 h-4" /></button>
                              }
                              <button onClick={e => { e.stopPropagation(); del(n.id); }} className="p-1.5 text-gray-400 hover:text-red-600 hover:bg-red-50 rounded transition-colors" title="Delete">
