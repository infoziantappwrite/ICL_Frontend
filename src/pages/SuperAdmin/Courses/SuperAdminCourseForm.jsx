...
import {
  ...
  UserCheck, ChevronDown, Calendar, ExternalLink,
} from 'lucide-react';
...
const INITIAL_FORM = {
  ...
  trainerId: '',  // optional trainer assignment (all course types)
};
...
const ModuleItem = ({ mod, index, onChange, onRemove }) => {
  ...
  const addResource = () => {
    if (!resourceInput.url.trim()) return;
    onChange({ ...mod, resource: [...(mod.resource || []), { title: resourceInput.title.trim(), url: resourceInput.url.trim() }] });
      ...
      <div className="flex items-start gap-2.5">
        ...
        <div className="flex-1 space-y-3">
          ...
          <div className="grid grid-cols-2 gap-2">
            <div>
              <p className="text-[10px] font-bold text-gray-500 mb-1 flex items-center gap-1 uppercase tracking-wider">
                <Calendar className="w-3 h-3 text-blue-400" /> Module Start
              </p>
              <input type="date" value={mod.startDate ? mod.startDate.split('T')[0] : ''}
                onChange={e => onChange({ ...mod, startDate: e.target.value })}
                className={inputClass} />
            </div>
            <div>
              <p className="text-[10px] font-bold text-gray-500 mb-1 flex items-center gap-1 uppercase tracking-wider">
                <Calendar className="w-3 h-3 text-red-400" /> Module End
              </p>
              <input type="date" value={mod.endDate ? mod.endDate.split('T')[0] : ''}
                onChange={e => onChange({ ...mod, endDate: e.target.value })}
                className={inputClass} />
            </div>
          </div>
          ...
          <div>
            <p className="text-[10px] font-bold text-gray-500 mb-1.5 flex items-center gap-1 uppercase tracking-wider">
              <ExternalLink className="w-3 h-3 text-emerald-500" /> Live Meeting URL
            </p>
            <input type="url" placeholder="https://meet.google.com/... or Zoom link"
              value={mod.liveMeetingUrl || ''}
              onChange={e => onChange({ ...mod, liveMeetingUrl: e.target.value })}
              className={inputClass} />
          </div>
          ...
          <div>
            <p className="text-[10px] font-bold text-gray-500 mb-1.5 flex items-center gap-1 uppercase tracking-wider">
              <BookOpen className="w-3 h-3 text-orange-500" /> Resource Links
            </p>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 mb-2">
              <input type="text" placeholder="Resource title (optional)" value={resourceInput.title}
                onChange={e => setResourceInput(p => ({ ...p, title: e.target.value }))} className={inputClass} />
              <div className="flex gap-2">
                <input type="url" placeholder="Resource URL *" value={resourceInput.url}
                  onChange={e => setResourceInput(p => ({ ...p, url: e.target.value }))}
                  onKeyDown={e => { if (e.key === 'Enter') { e.preventDefault(); addResource(); } }} className={inputClass} />
                <button type="button" onClick={addResource}
                  className="px-2.5 py-2 bg-orange-50 text-orange-600 border border-orange-100 rounded-xl hover:bg-orange-100 flex-shrink-0">
                  <Plus className="w-3.5 h-3.5" />
                </button>
              </div>
            </div>
            {(mod.resource || []).length > 0 && (
              <div className="space-y-1.5">
                {mod.resource.map((r, j) => (
                  <div key={j} className="flex items-center gap-2 bg-white border border-gray-100 rounded-lg px-3 py-1.5 group">
                    <ExternalLink className="w-3 h-3 text-orange-400 flex-shrink-0" />
                    <span className="text-[11px] text-gray-600 flex-1 min-w-0 truncate">
                      {r.title && <span className="font-semibold mr-1">{r.title} —</span>}
                      <a href={r.url} target="_blank" rel="noreferrer" className="text-blue-500 hover:underline">{r.url}</a>
                    </span>
                    <button type="button"
                      onClick={() => onChange({ ...mod, resource: mod.resource.filter((_, k) => k !== j) })}
                      className="text-gray-300 hover:text-red-500 flex-shrink-0 opacity-0 group-hover:opacity-100 transition-all">
                      <X className="w-3 h-3" />
                    </button>
                  </div>
      ...
      <div className={`flex items-start gap-2.5 p-3 rounded-xl border transition-all ${isPlatformWide ? 'bg-green-50 border-green-200' : 'bg-blue-50 border-blue-200'}`}>
        ...
        <div>
          ...
          <p className={`text-[10px] mt-0.5 ${isPlatformWide ? 'text-green-600' : 'text-blue-600'}`}>
            {isPlatformWide
              ? 'All colleges can access this course.'
              : 'Only students from selected colleges can access this course.'}
          </p>
        </div>
      </div>
...
// ── Trainer Picker — available for ALL course types ──────────────────────────
const TrainerPicker = ({ selectedId, onChange, trainers, loading, collegeCount }) => {
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex items-start gap-2.5 p-3 rounded-xl border bg-purple-50 border-purple-200">
        ...
        <div>
          <p className="text-xs font-bold text-purple-700">
            {selectedId
              ? `Assigned: ${selected?.fullName || 'Unknown Trainer'}`
              : 'No Trainer Assigned (Optional)'}
          </p>
          <p className="text-[10px] mt-0.5 text-purple-600">
            {collegeCount > 0
              ? `This trainer will deliver the course for the ${collegeCount} selected college(s).`
              : 'This trainer will deliver the platform-wide course.'}
          </p>
        </div>
      </div>
      ...
      <p className="text-[10px] text-gray-400 flex items-center gap-1">
        ...
        Trainer is optional — you can assign or change them after course creation too.
      </p>
    </div>
  );
};
...
const SuperAdminCourseForm = () => {
      ...
      if (res.success) {
        ...
        setForm({
          ...
          curriculum: (c.curriculum || []).map(m => ({
            ...m,
            videoUrl: m.videoUrl || '',
            liveMeetingUrl: m.liveMeetingUrl || '',
            videos: m.videos || [],
            resource: m.resource || [],
          ...
          isRegistrationClosed: c.isRegistrationClosed || false,
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const payload = {
        ...form,
        trainerId: undefined, // trainer assigned separately via /assign-trainer endpoint
      ...
      // ── Trainer assignment — works for ALL course types (platform-wide & college-scoped) ──
      // The backend handles re-assignment atomically: if a different trainer was previously
      // assigned, the backend unassigns them and assigns the new one in a single call.
      if (savedCourseId && form.trainerId) {
        const existingTrainerId = isEdit
          ? (res.data?.trainerId?._id || res.data?.trainerId || '')
          : '';

        // Only call assign-trainer if trainer changed (or it's a new course)
      ...
      } else if (isEdit && !form.trainerId) {
        // Trainer was removed in edit mode — explicitly unassign
        ...
        if (existingTrainerId) {
          ...
          } catch { /* best effort — course is saved, trainer removal is non-critical */ }
        }
      }
  ...
  return (
    <SuperAdminDashboardLayout>
      <div className="px-6 py-4 md:px-8 md:py-6 space-y-5 font-sans">
        ...
        {/* Trainer Assignment — available for ALL course types */}
        <Section title="Assign Trainer" icon={UserCheck} sub="Optionally assign a trainer to deliver this course">
          <TrainerPicker
            selectedId={form.trainerId}
            onChange={v => set('trainerId', v)}
            trainers={trainers}
            loading={trainersLoading}
            collegeCount={form.collegeIds.length}
          />
        </Section>
        ...
        {/* Curriculum */}
        <Section title="Course Curriculum" icon={Layers} sub="Modules, topics, videos, live sessions, and resources" noBg>
          <p className="text-[10px] text-gray-400 -mt-2">Each module supports start/end dates, a live meeting URL, a primary video, additional videos, and downloadable resource links.</p>
          <div className="space-y-3">
            ...
            <button type="button"
              onClick={() => set('curriculum', [...form.curriculum, {
                module: '', topics: [], duration: 0,
                videoUrl: '', liveMeetingUrl: '', videos: [], resource: [],
                startDate: '', endDate: '',
              }])}
        ...
        {/* Dates & Settings */}
        <Section title="Dates & Settings" icon={Calendar} sub="Scheduling, enrollment controls, and certificate options">
          ...
          {/* isRegistrationClosed toggle */}
          <div
            className={`flex items-center gap-3 p-3 rounded-xl border cursor-pointer transition-all ${form.isRegistrationClosed ? 'bg-red-50 border-red-200' : 'bg-gray-50 border-gray-200'}`}
            onClick={() => set('isRegistrationClosed', !form.isRegistrationClosed)}>
            <div className={`relative w-9 h-5 rounded-full flex-shrink-0 transition-colors duration-200 ${form.isRegistrationClosed ? 'bg-red-400' : 'bg-gray-300'}`}>
              <span className={`absolute top-0.5 w-4 h-4 bg-white rounded-full shadow-sm transition-transform duration-200 ${form.isRegistrationClosed ? 'translate-x-[18px]' : 'translate-x-0.5'}`} />
            </div>
            <div>
              <label className={`text-xs font-bold cursor-pointer ${form.isRegistrationClosed ? 'text-red-700' : 'text-gray-600'}`}>
                Registration Closed
              </label>
              <p className="text-[10px] text-gray-400 mt-0.5">
                {form.isRegistrationClosed
                  ? 'New enrollments are blocked regardless of deadline.'
                  : 'New enrollments are allowed (subject to deadline).'}
              </p>
            </div>
          </div>
