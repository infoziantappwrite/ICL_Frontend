...
const Section = ({ title, icon: Icon, iconBg, children }) => (
  <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
    <div className="flex items-center gap-3 mb-5 pb-4 border-b border-gray-50">
      <div className={`w-9 h-9 rounded-xl flex items-center justify-center ${iconBg}`}><Icon className="w-5 h-5" /></div>
  ...
  return (
    <div>
      <div className="flex gap-2 mb-2">
        ...
        <button type="button" onClick={add} className="px-3 py-2.5 bg-blue-100 text-blue-600 rounded-xl hover:bg-blue-200"><Plus className="w-4 h-4" /></button>
      </div>
      {values.length > 0 && (
        <div className="flex flex-wrap gap-2">
          {values.map((v, i) => (
            <span key={i} className="flex items-center gap-1.5 bg-gray-100 text-gray-700 text-sm px-3 py-1 rounded-full">
              {v}<button type="button" onClick={() => onChange(values.filter((_, j) => j !== i))} className="text-gray-400 hover:text-red-500"><X className="w-3.5 h-3.5" /></button>
            </span>
          ))}
        </div>
      )}
  ...
  return (
    <div className="border border-gray-200 rounded-xl p-4 bg-gray-50">
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 space-y-3">
          ...
          <div>
            ...
            <div className="flex gap-2 mb-2">
              ...
              <button type="button" onClick={addTopic} className="px-3 py-2 bg-blue-100 text-blue-600 rounded-xl hover:bg-blue-200"><Plus className="w-4 h-4" /></button>
            </div>
            {mod.topics?.length > 0 && (
              <div className="flex flex-wrap gap-1.5">
                {mod.topics.map((t, j) => (
                  <span key={j} className="flex items-center gap-1 bg-white text-gray-700 text-xs px-2 py-1 rounded-full border border-gray-200">
                    {t}<button type="button" onClick={() => onChange({ ...mod, topics: mod.topics.filter((_, k) => k !== j) })} className="text-gray-400 hover:text-red-500"><X className="w-3 h-3" /></button>
                  </span>
                ))}
              </div>
            )}
          ...
          <div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 mb-2">
              ...
              <div className="flex gap-2">
                ...
                <button type="button" onClick={addVideo} className="px-3 py-2 bg-indigo-100 text-indigo-600 rounded-xl hover:bg-indigo-200 flex-shrink-0"><Plus className="w-4 h-4" /></button>
              </div>
            </div>
            {(mod.videos || []).length > 0 && (
                ...
                {mod.videos.map((v, j) => (
                  <div key={j} className="flex items-center gap-2 bg-white border border-gray-100 rounded-lg px-3 py-2 group">
                    ...
                    <span className="text-xs text-gray-600 flex-1 min-w-0">
                      ...
                      <a href={v.url} target="_blank" rel="noreferrer" className="text-blue-500 hover:underline truncate">{v.url}</a>
                    </span>
        ...
        <button type="button" onClick={onRemove} className="p-1.5 text-gray-400 hover:text-red-500 hover:bg-red-50 rounded-lg mt-1"><Trash2 className="w-4 h-4" /></button>
      </div>
    </div>
  );
};
...
const SuperAdminCourseForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = isEdit ? await superAdminCourseAPI.updateCourse(courseId, payload) : await superAdminCourseAPI.createCourse(payload);
      if (res.success) {
  ...
  return (
    <DashboardLayout title={isEdit ? 'Edit Course' : 'Create Course'}>
      ...
      <div className="flex items-center justify-between mb-6">
        <button onClick={() => navigate('/dashboard/super-admin/courses')} className="flex items-center gap-2 text-sm text-gray-500 hover:text-blue-600">
          <ChevronLeft className="w-4 h-4" /> Back to Courses
        </button>
        <button onClick={handleSubmit} disabled={saving} className="flex items-center gap-2 px-5 py-2.5 bg-gradient-to-r from-blue-600 to-cyan-600 text-white font-semibold rounded-xl shadow-lg disabled:opacity-60">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        {/* Platform Scope */}
        <Section title="Platform Scope" icon={Globe} iconBg="bg-blue-100 text-blue-600">
          <div className="flex items-start gap-4 p-4 bg-blue-50 border border-blue-200 rounded-xl">
            ...
            <div className="flex-1">
              <p className="font-semibold text-blue-800 text-sm mb-1">Course Visibility</p>
              <p className="text-xs text-blue-600 mb-3">Leave College ID blank to make this course available to all colleges (platform-wide).</p>
              <FormField label="College ID (optional)" hint="Leave blank for platform-wide visibility">
                <input type="text" value={form.collegeId} onChange={e => set('collegeId', e.target.value)} className={inputClass} placeholder="MongoDB ObjectId of specific college, or leave blank" />
              </FormField>
            </div>
          </div>
        </Section>
        ...
        <Section title="Basic Information" icon={BookOpen} iconBg="bg-blue-100 text-blue-600">
          <FormField label="Course Title" required>
            <input type="text" value={form.title} onChange={e => set('title', e.target.value)} className={inputClass} placeholder="e.g., Full Stack Web Development" />
            {errors.title && <p className="text-xs text-red-500 mt-1">{errors.title}</p>}
          </FormField>
          <FormField label="Short Description" hint="Max 200 characters">
            <textarea value={form.shortDescription} onChange={e => set('shortDescription', e.target.value)} className={inputClass} rows={2} maxLength={200} />
            <p className="text-xs text-gray-400 mt-1">{form.shortDescription.length}/200</p>
          </FormField>
          <FormField label="Full Description" required>
            <textarea value={form.description} onChange={e => set('description', e.target.value)} className={inputClass} rows={5} />
        ...
        <Section title="Course Curriculum" icon={Layers} iconBg="bg-cyan-100 text-cyan-600">
          <p className="text-xs text-gray-400 -mt-2 mb-1">Each module supports a primary video URL and multiple additional videos.</p>
          <div className="space-y-3">
            ...
            <button type="button"
              ...
              className="w-full flex items-center justify-center gap-2 py-3 border-2 border-dashed border-blue-200 text-blue-600 rounded-xl hover:border-blue-400 hover:bg-blue-50 text-sm font-medium">
        ...
        <Section title="Media & Resources" icon={BookOpen} iconBg="bg-indigo-100 text-indigo-600">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <FormField label="Thumbnail URL"><input type="url" value={form.thumbnail} onChange={e => set('thumbnail', e.target.value)} className={inputClass} placeholder="https://..." /></FormField>
            <FormField label="Preview Video URL"><input type="url" value={form.videoUrl} onChange={e => set('videoUrl', e.target.value)} className={inputClass} placeholder="YouTube embed URL" /></FormField>
            <FormField label="Syllabus PDF URL"><input type="url" value={form.syllabusUrl} onChange={e => set('syllabusUrl', e.target.value)} className={inputClass} placeholder="https://..." /></FormField>
          </div>
        </Section>
        ...
        <Section title="Dates & Settings" icon={Award} iconBg="bg-rose-100 text-rose-600">
          ...
          <div className="flex items-center gap-3 p-4 bg-gray-50 rounded-xl">
            ...
            <label htmlFor="cert" className="text-sm font-medium text-gray-700 cursor-pointer">Provide certificate upon completion</label>
          </div>
        </Section>

        <div className="flex justify-end gap-3 pt-2">
          <button type="button" onClick={() => navigate('/dashboard/super-admin/courses')} className="px-5 py-2.5 border border-gray-200 text-gray-700 rounded-xl font-medium text-sm hover:bg-gray-50">Cancel</button>
          <button type="submit" disabled={saving} className="flex items-center gap-2 px-6 py-2.5 bg-gradient-to-r from-blue-600 to-cyan-600 text-white font-semibold rounded-xl shadow-lg disabled:opacity-60">
