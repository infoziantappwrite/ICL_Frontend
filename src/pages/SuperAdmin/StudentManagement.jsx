...
// Uses ONLY real backend: /api/super-admin/students/*
...
const Overlay = ({ children, onClose, size='md' }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" onClick={onClose}>
      ...
      <div className={`relative w-full ${w} max-h-[90vh] flex flex-col`} onClick={e=>e.stopPropagation()}>
        {children}
      </div>
    </div>
  );
      ...
      <div className="bg-white rounded-2xl overflow-hidden flex flex-col max-h-[90vh]">
        <div className="bg-gradient-to-r from-indigo-600 to-violet-600 px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-base font-bold text-white">Bulk Student Upload</h2>
              <p className="text-xs text-indigo-200">Cross-college student upload · max 500/file</p>
            </div>
          </div>
        ...
        {/* Step pills */}
        <div className="px-6 pt-4 pb-1 flex items-center gap-1">
          {STEPS.map((s,i) => {
        ...
        <div className="p-6 overflow-y-auto flex-1 space-y-4">
          ...
          <div className="flex items-center gap-3 p-3 bg-slate-50 border border-slate-200 rounded-xl">
            ...
            <p className="flex-1 text-xs text-slate-500">Super Admin can leave college_id blank in file if setting a default college below.</p>
          ...
          <div>
            <label className="text-xs font-bold text-slate-700 block mb-1.5">
              Default College <span className="font-normal text-slate-400">(optional — row's college_id overrides this)</span>
            </label>
            <div className="relative">
              <Building2 size={13} className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 pointer-events-none"/>
              <select value={cid} onChange={e=>setCid(e.target.value)}
                className="w-full text-sm border border-slate-200 rounded-xl pl-9 pr-4 py-2.5 bg-white focus:outline-none focus:ring-2 focus:ring-indigo-400">
                <option value="">None — each row must include college_id</option>
          ...
          {step <= 1 && (
            <div className="space-y-3">
              <div
                ...
                {file ? (
                  <><CheckCircle size={32} className="text-emerald-500"/><p className="text-sm font-bold text-emerald-700">{file.name}</p><p className="text-xs text-slate-400">{(file.size/1024).toFixed(1)} KB — click to replace</p></>
                ) : (
            ...
            <div className="space-y-4">
              ...
              {vr.validationErrors?.length>0 && (
                <div className="max-h-32 overflow-y-auto space-y-1">
                  <p className="text-xs font-bold text-red-500 uppercase tracking-wide">Errors:</p>
              ...
              {vr.warnings?.map((w,i) => (
                <div key={i} className="flex gap-2 text-xs text-amber-700 bg-amber-50 rounded-lg px-3 py-1.5 border border-amber-100"><AlertTriangle size={10} className="flex-shrink-0 mt-0.5"/>{w}</div>
              ))}
              {vr.preview?.length>0 && (
            ...
            <div className="col-span-2">
              ...
              <div className="relative">
                ...
                <select value={f.collegeId} onChange={e=>setF('collegeId',e.target.value)}
                  className="w-full text-sm border border-slate-200 rounded-xl pl-9 py-2.5 bg-white focus:outline-none focus:ring-2 focus:ring-emerald-400">
...
// ── Feature card ───────────────────────────────────────────────────────────────
const FeatureCard = ({ icon:Icon, gradient, title, desc, tips, onOpen, openLabel }) => (
  <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
    <div className={`${gradient} px-5 py-4 flex items-center gap-3`}>
      <div className="w-9 h-9 bg-white/20 rounded-xl flex items-center justify-center"><Icon size={18} className="text-white"/></div>
    ...
    <div className="p-5 space-y-3">
      ...
      <button onClick={onOpen}
        className="w-full mt-2 flex items-center justify-center gap-2 py-2.5 text-sm font-bold text-white rounded-xl transition-colors"
        style={{background:'linear-gradient(135deg,var(--c1),var(--c2))'}}
        onMouseEnter={e=>{e.currentTarget.style.opacity='0.9';}}
        onMouseLeave={e=>{e.currentTarget.style.opacity='1';}}>
        <Icon size={14}/>{openLabel}
      </button>
    </div>
  </div>
);
...
export default function SuperAdminStudentManagement() {
  ...
  const [modal,    setModal]    = useState(null); // 'upload'|'export'
      ...
      <div className="max-w-5xl mx-auto space-y-6">
        ...
        <div className="flex items-start justify-between gap-4 flex-wrap">
          <div>
            ...
            <p className="text-sm text-slate-400 mt-0.5">Global operations — bulk upload and export across all colleges</p>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
            <div className="bg-gradient-to-r from-indigo-600 to-violet-600 px-5 py-4 flex items-center gap-3">
              <div className="w-9 h-9 bg-white/20 rounded-xl flex items-center justify-center"><CloudUpload size={18} className="text-white"/></div>
              <h3 className="text-base font-bold text-white">Bulk Upload</h3>
            </div>
            <div className="p-5 space-y-3">
              <p className="text-sm text-slate-500 leading-relaxed">Upload hundreds of students at once. Supports cross-college uploads with an optional default college or per-row college_id.</p>
              <ul className="space-y-2">
                {['Download the Excel template with field descriptions','Fill student data — name, email, password, roll_number required','Set a default college OR include college_id column per row','Validate first — errors shown before any data is saved','Database transaction — all rows succeed or none do'].map((t,i)=>(
                  <li key={i} className="flex items-start gap-2 text-xs text-slate-600">
                    <div className="w-4 h-4 bg-indigo-100 text-indigo-700 rounded-full flex items-center justify-center text-xs font-bold flex-shrink-0 mt-0.5">{i+1}</div>
                    {t}
                  </li>
                ))}
              </ul>
              <button onClick={()=>setModal('upload')}
                className="w-full flex items-center justify-center gap-2 py-2.5 text-sm font-bold text-white bg-gradient-to-r from-indigo-600 to-violet-600 hover:opacity-90 rounded-xl transition-opacity">
                <CloudUpload size={14}/> Open Bulk Upload
              </button>
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
            ...
            <div className="p-5 space-y-3">
              <p className="text-sm text-slate-500 leading-relaxed">Export student records across all colleges or filtered by college, branch, batch, or placement status.</p>
              <ul className="space-y-2">
                {['Filter by college, branch, batch, or placement status','Preview the record count before downloading','Excel export includes 3 sheets: data · branch stats · metadata','CSV export gives a simple flat table for easy processing'].map((t,i)=>(
                  <li key={i} className="flex items-start gap-2 text-xs text-slate-600">
                    <ChevronRight size={12} className="mt-0.5 flex-shrink-0 text-slate-300"/>{t}
                  </li>
                ))}
              </ul>
              <button onClick={()=>setModal('export')}
                className="w-full flex items-center justify-center gap-2 py-2.5 text-sm font-bold text-white bg-gradient-to-r from-emerald-500 to-teal-500 hover:opacity-90 rounded-xl transition-opacity">
        ...
        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-6">
          <div className="flex items-center gap-2 mb-5">
            ...
            <h3 className="text-sm font-bold text-slate-800">Upload Template Column Reference</h3>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            ...
            <div>
              ...
              <div className="space-y-2">
                {[
                  ['college_id', 'MongoDB ObjectId (required for super admin without default)'],
                  ['role',       'student or candidate (default: student)'],
          ...
          <div className="mt-5 p-3 bg-amber-50 border border-amber-200 rounded-xl text-xs text-amber-700 flex gap-2">
            <AlertTriangle size={13} className="flex-shrink-0 mt-0.5"/>
            <span>If <strong>any</strong> row fails validation, the <strong>entire upload is rejected</strong>. Fix all errors shown in the Validate step, then re-upload.</span>
          </div>
        </div>
      </div>
