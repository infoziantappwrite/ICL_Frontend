...
import {
  CreditCard, Building2, RefreshCw, Pencil, X, CircleCheck,
    ...
    <SuperAdminDashboardLayout>
      ...
      <div className="relative bg-gradient-to-r from-blue-700 via-blue-600 to-cyan-500 rounded-2xl px-5 py-4 mb-4 shadow-xl shadow-blue-500/20 overflow-hidden">
        ...
        <div className="relative flex items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 bg-white/20 rounded-xl flex items-center justify-center flex-shrink-0"><CreditCard className="w-5 h-5 text-white"/></div>
            <div>
              <h1 className="text-white font-black text-lg leading-tight">Subscription Management</h1>
              <p className="text-blue-200 text-[11px] mt-0.5">Manage college plans, limits & billing · {colleges.length} colleges</p>
            </div>
          </div>
          <div className="flex-shrink-0 text-right hidden sm:block">
            <p className="text-white font-black text-xl leading-none">₹{totalRevenue.toLocaleString()}</p>
            <p className="text-blue-200 text-[10px] mt-0.5">Monthly Revenue (Mock)</p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white/80 backdrop-blur-xl rounded-2xl border border-white/60 shadow-sm p-3 mb-4">
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-2">
          {[
            { label:'Total Colleges',  value: colleges.length,          color:'bg-blue-50 border-blue-100 text-blue-600',    icon:Building2    },
            { label:'Free',            value: planCounts.free,          color:'bg-gray-50 border-gray-200 text-gray-600',    icon:Shield       },
            { label:'Pro',             value: planCounts.pro,           color:'bg-blue-50 border-blue-100 text-blue-600',    icon:Zap          },
            { label:'Pro Plus',        value: planCounts.pro_plus,      color:'bg-indigo-50 border-indigo-100 text-indigo-600',icon:Crown       },
        ...
        <div className="flex gap-2 flex-wrap">
          {[['all','All Plans'],['free','Free'],['pro','Pro'],['pro_plus','Pro Plus']].map(([key,label]) => (
            <button key={key} onClick={() => setFilterPlan(key)}
              className={`px-4 py-2 rounded-xl text-xs font-bold transition-all ${filterPlan===key ? 'bg-gradient-to-r from-blue-600 to-cyan-500 text-white shadow-sm' : 'bg-gray-100 text-gray-600 hover:bg-blue-50 hover:text-blue-600'}`}>
              {label}
            </button>
          ))}
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        {filtered.map(college => {
          ...
          return (
            <div key={college._id}
              className={`bg-white/80 backdrop-blur-xl rounded-2xl border-2 shadow-sm p-4 transition-all hover:shadow-md ${cfg.ring}`}
            >
              <div className="flex items-start justify-between mb-3">
                <div className="flex items-center gap-2.5">
                  <div className="w-9 h-9 bg-gradient-to-br from-blue-500 to-cyan-500 rounded-xl flex items-center justify-center text-white text-[10px] font-black flex-shrink-0 shadow-sm">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button onClick={() => setEditing(college)}
                    className="p-1.5 text-blue-500 hover:bg-blue-50 rounded-lg transition-colors" title="Edit plan">
              ...
              <div className="flex items-center justify-between pt-2 border-t border-gray-100">
                ...
                <span className="text-xs font-black text-blue-700 bg-blue-50 px-2.5 py-1 rounded-lg border border-blue-100">{college.amount}</span>
              </div>
      ...
      <div className="mt-4 bg-gradient-to-br from-blue-600 via-blue-600 to-cyan-500 rounded-2xl p-4 text-white shadow-lg shadow-blue-500/20 relative overflow-hidden">
        <div className="absolute -top-6 -right-6 w-20 h-20 bg-white/10 rounded-full"/>
        <div className="absolute -bottom-4 -left-4 w-16 h-16 bg-white/10 rounded-full"/>
        <div className="relative grid grid-cols-2 sm:grid-cols-4 gap-4">
          <div className="text-center">
            <p className="text-2xl font-black">₹{totalRevenue.toLocaleString()}</p>
            <p className="text-blue-200 text-[10px] mt-0.5">Monthly MRR</p>
          </div>
          <div className="text-center">
            <p className="text-2xl font-black">₹{(totalRevenue*12).toLocaleString()}</p>
            <p className="text-blue-200 text-[10px] mt-0.5">Annual ARR</p>
          </div>
          <div className="text-center">
            <p className="text-2xl font-black">{planCounts.pro + planCounts.pro_plus}</p>
            <p className="text-blue-200 text-[10px] mt-0.5">Paid Colleges</p>
          </div>
          <div className="text-center">
            <p className="text-2xl font-black">{colleges.length > 0 ? Math.round(((planCounts.pro+planCounts.pro_plus)/colleges.length)*100) : 0}%</p>
            <p className="text-blue-200 text-[10px] mt-0.5">Conversion Rate</p>
          </div>
        </div>
      </div>
