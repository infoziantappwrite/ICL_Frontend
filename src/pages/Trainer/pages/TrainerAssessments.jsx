// pages/Trainer/pages/TrainerAssessments.jsx
import { useState, useEffect } from 'react';
import { useNavigate } from 'react-router-dom';
import {
  Plus, Search, SquarePen, Trash2, ChartBar,
  ListChecks, Users, RefreshCw, AlertCircle,
  CircleCheck, Clock, Ban, Play, CalendarCheck,
  ChevronLeft, ChevronRight, X,
  Hash, Award, Calendar, BookOpen, Layers,
} from 'lucide-react';
...
const TrainerAssessments = () => {
  ...
  const fetchAssessments = async () => {
    ...
    try {
      ...
      const list = (res.assessments || []).sort((a, b) => new Date(b.createdAt || 0) - new Date(a.createdAt || 0));
      setAssessments(list);
    } catch (err) {
  ...
  return (
    <TrainerDashboardLayout>
      <div className="px-2 py-4 md:px-4 md:py-6 space-y-5 font-sans">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-2">
          <div>
            <h1 className="text-2xl font-black text-slate-900 tracking-tight flex items-center gap-2">
              ...
              My <span className="text-[#003399]">Assessments</span>
            </h1>
            <p className="text-sm text-slate-400 mt-1 font-medium">Manage and track your assessments — {stats.total} total</p>
          </div>
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <button onClick={() => navigate('/dashboard/trainer/assessments/create')}
        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-100 flex flex-col overflow-hidden">
          ...
          {error && (
            <div className="mx-4 mt-4 flex items-center gap-2 bg-red-50 border border-red-200 rounded-xl p-3 text-sm text-red-600">
            ...
            {loading ? (
              ...
              <div className="text-center py-24">
                ...
                <p className="text-slate-400 text-sm mb-6 max-w-[300px] mx-auto">
                  {search || statusFilter ? 'Try adjusting your search or filter.' : 'Create your first assessment to start evaluating students.'}
                </p>
                {!search && !statusFilter && (
                  <button onClick={() => navigate('/dashboard/trainer/assessments/create')}
                ...
                <table className="w-full table-fixed">
                  <thead>
                    <tr className="bg-slate-50/50 border-b border-slate-100">
                      <th className="px-5 py-4 text-[10px] font-black text-slate-400 uppercase tracking-widest text-left w-[56px]">S.No</th>
                      ...
                      <th className="px-5 py-4 text-[10px] font-black text-slate-400 uppercase tracking-widest text-left w-[160px]">Status</th>
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {paginated.map((a, index) => {
                      ...
                      const qCount     = Array.isArray(a.questions_id) ? a.questions_id.length : (a.num_questions || 0);
                      ...
                      return (
                        <tr key={a._id} className="hover:bg-slate-50/20 transition-colors group">
                          <td className="px-5 py-4 text-xs font-bold text-slate-400">{String(sNo).padStart(2, '0')}</td>

                          {/* Title + Level */}
                          <td className="px-5 py-4">
                            <div className="min-w-0">
                              <p className="text-sm font-bold text-slate-800 truncate w-full">{a.title || a.skill_id?.name || 'Untitled'}</p>
                              {a.level && (
                              ...
                              {a.skill_id?.name && (
                                <p className="text-[10px] text-slate-400 font-semibold mt-0.5 truncate">{a.skill_id.name}</p>
                              )}
                            </div>
                          </td>
                          ...
                          <td className="px-5 py-4">
                            <div className="flex flex-wrap items-center gap-2 text-[10px] font-black uppercase tracking-widest text-slate-400">
                              ...
                              <span className="flex items-center gap-1 bg-slate-50 px-1.5 py-0.5 rounded-md border border-slate-100">
                                <Users className="w-2.5 h-2.5 text-indigo-400" />{a.eligible_students?.length || a.totalAttempts || 0}
                              </span>
                            </div>
                            {a.scheduled_date && (
                            ...
                            <p className="text-[10px] text-slate-300 font-medium mt-1">
                              Created {new Date(a.createdAt).toLocaleDateString('en-IN', { day: '2-digit', month: 'short', year: 'numeric' })}
                            </p>
                          </td>
                          ...
                          {/* Actions — FIXED: each menu item goes to its correct route */}
                            ...
                            <div className="flex items-center justify-center">
                              <ActionMenu actions={[
                                canEdit && {
                                  label: 'Edit Details',
                                  icon: SquarePen,
                                  onClick: () => navigate(`/dashboard/trainer/assessments/${a._id}/manage`),
                                  color: 'text-amber-600 hover:bg-amber-50',
                                },
                                {
                                  label: 'Manage Sections',
                                  icon: Layers,
                                  onClick: () => navigate(`/dashboard/trainer/assessments/${a._id}/sections`),
                                  color: 'text-[#003399] hover:bg-slate-50',
                                },
                                {
                                  label: 'Manage Questions',
                                  icon: ListChecks,
                                  onClick: () => navigate(`/dashboard/trainer/assessments/${a._id}/questions`),
                                  color: 'text-violet-600 hover:bg-violet-50',
                                },
                                {
                                  label: 'View Attempts',
                                  icon: ChartBar,
                                  onClick: () => navigate(`/dashboard/trainer/assessments/${a._id}/results`),
